...
class Enemy {
  ...
  draw(ctx, sprite) {
    if (sprite) ctx.drawImage(sprite, this.x - this.w / 2, this.y - this.h / 2, this.w, this.h);
    else { ctx.fillStyle = '#D96B2B'; ctx.fillRect(this.x - this.w / 2, this.y - this.h / 2, this.w, this.h); ctx.fillStyle = '#8F3E14'; ctx.fillRect(this.x - 4, this.y - this.h / 2, 8, this.h); }
  }
}
...
class Game {
  ...
  drawBackground() {
    ...
    if (bg) { c.drawImage(bg, 0, this.scrollY - this.height, this.width, this.height); c.drawImage(bg, 0, this.scrollY, this.width, this.height); return; }
  ...
  draw() { this.drawBackground(); this.player.draw(this.ctx); [...this.bullets, ...this.enemyBullets].forEach(b => b.draw(this.ctx)); this.enemies.forEach(e => e.draw(this.ctx, this.getEnemySprite(e.type === 'small' ? 'small' : 'heavy'))); this.powerUps.forEach(p => p.draw(this.ctx)); this.ui.drawUI(); }
